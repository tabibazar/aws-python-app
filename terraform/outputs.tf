output "service_hostname" {
  ...
  value = try(
    kubernetes_service_v1.svc.status[0].load_balancer[0].ingress[0].hostname,
...
output "service_ip" {
  ...
  value = try(
    kubernetes_service_v1.svc.status[0].load_balancer[0].ingress[0].ip,
    kubernetes_service_v1.flask_app[0].status[0].load_balancer[0].ingress[0].ip,
    kubernetes_service_v1.svc.spec[0].cluster_ip,
